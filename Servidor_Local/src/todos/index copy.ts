...
app.delete("/apagar-utilizador", async (req: Request, res: Response) => {
  const { id } = req.body
  const apagarUtilizadorResponse = await deleteUser(id as string)
  res.json(apagarUtilizadorResponse)
})
